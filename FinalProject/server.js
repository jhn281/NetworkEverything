...
var ARDUINO_IP_ADDRESS = '10.225.161.130'; // IMNetwork
// var ARDUINO_IP_ADDRESS = '10.225.154.176'; // Showcase
...
function ArduinoUDPServerReceivedMessage(message, sender) {
	...
	if(message.readUInt8(0) == 1) {
		console.log("received message to take a pic");
		webSocket.emit('takeSnap');
	}

	if(0 <= message.readUInt8(1) && message.readUInt8(1) <= 5) {
		console.log("received message to change Filter: "+message.readUInt8(1));
		webSocket.emit('changeFilter', message.readUInt8(1));
	}

	if(0 <= message.readUInt8(2) && message.readUInt8(2) <= 5) {
		console.log("received message to change Intensity: "+message.readUInt8(2));
		webSocket.emit('changeIntensity', message.readUInt8(2));
	}
